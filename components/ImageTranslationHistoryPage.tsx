...
export const ImageTranslationHistoryPage: React.FC<ImageTranslationHistoryPageProps> = ({
  ...
  <div className="h-full overflow-y-auto" style={{ background: '#241708', color: '#fff' }}>
    <header className="sticky top-0 z-10 flex items-center gap-3 border-b border-white/10 bg-[#241708]/95 px-4 pb-4 pt-[max(16px,env(safe-area-inset-top))] backdrop-blur">
      <button onClick={onBack} className="h-11 w-11 rounded-full bg-white/10 flex items-center justify-center active:scale-95" aria-label="返回"><ArrowLeft size={23} /></button>
      <div className="flex-1">
        <p className="text-2xl font-black tracking-wide">歷史</p>
        <p className="text-xs text-white/55">圖片翻譯紀錄</p>
      </div>
      <button onClick={onOpenCamera} className="h-11 w-11 rounded-full bg-white flex items-center justify-center text-[#6d3219] active:scale-95" aria-label="開始新的翻譯"><Camera size={22} /></button>
    </header>
    ...
    <main className="mx-auto w-full max-w-lg space-y-4 px-4 pb-12 pt-5">
      {records.length === 0 ? (
        <div className="flex min-h-[55vh] flex-col items-center justify-center text-center text-white/60">
          ...
          <p className="text-lg font-bold text-white/80">還沒有翻譯紀錄</p>
          <p className="mt-1 text-sm">拍攝或上傳菜單後，紀錄會顯示在這裡。</p>
        </div>
      ) : records.map(record => {
