...
const TRANSLATIONS: Record<string, any> = {
    '繁體中文': {
        ...
        footerDisclaimer: '費用將透過您的支付帳號一次性扣款，無任何自動續訂的隱藏費用。'
    },
    '繁體中文-HK': {
        title: '解鎖全功能(終身會員)',
        subtitle: '自備 API Key 每日無限次翻譯(依API每日額度)、享用所有功能、新功能更新不加價！',
    ...
    'English': {
        ...
        footerDisclaimer: 'One-time charge via App Store / Google Play. No hidden fees.'
    },
    '日本語': {
        title: '全機能解放（生涯会員）',
        subtitle: '自分のAPI Keyで毎日無制限翻譯（API上限内）。全機能利用可、アプデ追加料金なし！',
        features: ['日次制限なし、無制限翻訳', 'メニューライブラリ解放', '全注文履歴を解放', '一度の支払いで永久利用'],
        bestValue: 'お得',
        lifetime: '生涯会員',
        oneTimePay: '一度の支払いで永久アップデート',
        restore: '購入の復元',
        footerDisclaimer: 'Apple / Google Playへの1回限りの請求です。自動更新や隠れた費用はありません。'
    },
    '한국어': {
        title: '전체 기능 해제 (평생)',
        ...
        restore: '구매 복원',
        footerDisclaimer: 'App Store / Google Play를 통한 일회성 결제입니다.'
    },
    'ไทย': {
        ...
        footerDisclaimer: 'เรียกเก็บครั้งเดียวผ่าน App Store / Google Play'
    },
    'Tiếng Việt': {
        ...
        footerDisclaimer: 'Tính phí 1 lần qua App Store / Google Play.'
    }
};
    ...
    useEffect(() => {
        ...
        const loadOfferings = async () => {
            ...
            try {
                ...
                const appleKey = process.env.NEXT_PUBLIC_REVENUECAT_APPLE_KEY;
                const googleKey = process.env.NEXT_PUBLIC_REVENUECAT_GOOGLE_KEY;

                const apiKey = isIos ? appleKey : googleKey;

                if (!apiKey) {
                    console.warn(`[Paywall] RevenueCat API Key is missing for platform: ${isIos ? 'ios' : 'android'}`);
                    return;
                }

                // ✅ 取得當前登入用戶的 Email，用於跨平台購買辨識
                ...
                try {
                    // ✅ 使用 Email 作為 appUserID，讓 RevenueCat 在不同系統間同步
                    if (userEmail) {
                        await Purchases.configure({ apiKey, appUserID: userEmail });
                        console.log(`[Paywall] RevenueCat configured for ${isIos ? 'iOS' : 'Android'} with user:`, userEmail);
                    } else {
                        await Purchases.configure({ apiKey });
                        console.log(`[Paywall] RevenueCat configured for ${isIos ? 'iOS' : 'Android'} as anonymous`);
                    }
        ...
        try {
            ...
            if (typeof result.customerInfo.entitlements.active['pro'] !== "undefined") {
                ...
                if (userEmail) {
                    ...
                    }).catch(err => console.warn('[Paywall] Failed to sync PRO status:', err));
                }
                ...
                >
                    ...
                    >
                        ...
                        <div className="pt-8 pb-6 px-6 relative z-10 text-center">
                            <button onClick={onClose} className="absolute top-4 right-4 w-8 h-8 flex items-center justify-center bg-white/20 hover:bg-white/30 text-white rounded-full backdrop-blur-sm transition-colors"><i className="ph ph-x"></i></button>
                            <div className="w-20 h-20 rounded-full shadow-lg mx-auto flex items-center justify-center mb-4 mt-2" style={{ background: 'var(--bg-tertiary)', border: '3px solid var(--glass-border)' }}><i className="ph-fill ph-crown text-4xl" style={{ color: 'var(--brand-primary)' }}></i></div>
                            <h2 className="text-2xl font-bold mb-2" style={{ color: 'var(--text-primary)' }}>{t.title}</h2>
                            <p className="text-sm px-2" style={{ color: 'var(--text-secondary)' }}>{t.subtitle}</p>
                        </div>
                        <div className="px-8 pb-6 space-y-3">
                            {t.features.map((feature: string, i: number) => (
                                <div key={i} className="flex items-center gap-3">
                                    <div className="w-5 h-5 rounded-full flex items-center justify-center shrink-0" style={{ background: 'var(--brand-bg)' }}><i className="ph-bold ph-check text-xs" style={{ color: 'var(--brand-primary)' }}></i></div>
                        ...
                        <div className="px-6 pb-6 space-y-3 max-h-[40vh] overflow-y-auto">
                            {loading ? (
                                <div className="py-8 flex justify-center"><div className="w-8 h-8 border-4 border-orange-200 border-t-orange-500 rounded-full animate-spin"></div></div>
                            ) : offering?.availablePackages.map((pkg) => (
                                <button key={pkg.identifier} disabled={purchasing} onClick={() => handlePurchase(pkg)} className="w-full relative overflow-hidden group p-4 rounded-2xl text-left transition-all active:scale-[0.98]" style={{ background: 'var(--glass-bg)', border: '2px solid var(--glass-border)' }}>
                                    {pkg.identifier === '$rc_lifetime' && <div className="absolute top-0 right-0 text-white text-[10px] font-bold px-3 py-1 rounded-bl-lg" style={{ background: 'var(--brand-primary)' }}>{t.bestValue}</div>}
                                    <div className="flex justify-between items-center">
                                        <div>
                                            <h3 className="font-bold text-lg" style={{ color: 'var(--text-primary)' }}>{pkg.identifier === '$rc_lifetime' ? t.lifetime : 'Upgrade'}</h3>
                                            <p className="text-xs" style={{ color: 'var(--text-tertiary)' }}>{t.oneTimePay}</p>
                                        </div>
                                        <div className="text-right"><div className="text-xl font-bold" style={{ color: 'var(--brand-primary)' }}>{pkg.product.priceString}</div></div>
                                    </div>
                                </button>
                            ))}
                        </div>
                        <div className="px-6 pb-6 text-center">
                            <button onClick={handleRestore} disabled={purchasing} className="text-xs font-medium underline" style={{ color: 'var(--text-tertiary)' }}>{t.restore}</button>
                            <p className="text-[10px] mt-4 leading-relaxed" style={{ color: 'var(--text-muted)' }}>{t.footerDisclaimer}</p>
                        </div>
                    </motion.div>
                </motion.div>
            )}
