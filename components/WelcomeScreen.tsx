...
export const WelcomeScreen: React.FC<WelcomeScreenProps> = ({
    ...
    const promoText = (() => {
        ...
        if (lang === TargetLanguage.Japanese) return { title: 'SausageMenuPal', subtitle: '新 Android 版がリリースされました！', iosDev: 'iOS開発中、お楽しみに🥹', androidTitle: 'Android 版', androidDesc: 'Google Play ストアからダウンロード', downloadBtn: 'ダウンロード', lineTitle: 'Line コミュニティ参加', lineDesc: '旅行好きの仲間と交流', joinBtn: '参加する', later: '後で見る' };
        if (lang === TargetLanguage.Korean) return { title: 'SausageMenuPal', subtitle: '새로운 Android 버전 출시!', iosDev: 'iOS 개발 중, 기대해 주세요🥹', androidTitle: 'Android 버전', androidDesc: 'Google Play 스토어 다운로드', downloadBtn: '다운로드', lineTitle: 'Line 커뮤니티 참여', lineDesc: '여행을 좋아하는 분들과 교류', joinBtn: '참여하기', later: '나중에 보기' };
        if (lang === TargetLanguage.English) return { title: 'SausageMenuPal', subtitle: 'New Android version is live!', iosDev: 'iOS coming soon, stay tuned🥹', androidTitle: 'Android Version', androidDesc: 'Download from Google Play', downloadBtn: 'Download', lineTitle: 'Join Line Community', lineDesc: 'Connect with fellow travelers', joinBtn: 'Join Now', later: 'Maybe Later' };
        if (lang === TargetLanguage.Thai) return { title: 'SausageMenuPal', subtitle: 'เวอร์ชัน Android ใหม่มาแล้ว!', iosDev: 'iOS กำลังพัฒนา รอติดตามนะ🥹', androidTitle: 'เวอร์ชัน Android', androidDesc: 'ดาวน์โหลดจาก Google Play', downloadBtn: 'ดาวน์โหลด', lineTitle: 'เข้าร่วมกลุ่ม Line', lineDesc: 'แลกเปลี่ยนกับนักเดินทาง', joinBtn: 'เข้าร่วม', later: 'ไว้ทีหลัง' };
        if (lang === TargetLanguage.Vietnamese) return { title: 'SausageMenuPal', subtitle: 'Phiên bản Android mới đã ra mắt!', iosDev: 'iOS đang phát triển, hãy chờ đón🥹', androidTitle: 'Phiên bản Android', androidDesc: 'Tải từ Google Play', downloadBtn: 'Tải xuống', lineTitle: 'Tham gia cộng đồng Line', lineDesc: 'Kết nối với những người yêu du lịch', joinBtn: 'Tham gia', later: 'Để sau' };
        return { title: 'SausageMenuPal', subtitle: '全新 Android 版本上架囉！', iosDev: 'iOS 開發中，敬請等待🥹', androidTitle: 'Android 版本', androidDesc: 'Google Play 商店下載', downloadBtn: '點擊下載', lineTitle: '加入 Line 社群分享回饋', lineDesc: '與其他旅遊愛好者交流', joinBtn: '點擊加入', later: '稍後再看' };
    })();

    const contactText = (() => {
        const lang = uiLanguage;
        if (lang === TargetLanguage.Japanese) return { title: 'お問い合わせ', subtitle: 'ご質問があればお気軽にどうぞ！', waDesc: 'メッセージを送る', line: '匿名Lineコミュニティに参加', lineDesc: '旅行好きの仲間と交流', email: 'メールアドレス', close: '閉じる' };
        if (lang === TargetLanguage.Korean) return { title: '문의하기', subtitle: '궁금한 점이 있으시면 연락주세요!', waDesc: '메시지 보내기', line: '익명 Line 커뮤니티 참여', lineDesc: '여행 애호가들과 교류', email: '이메일', close: '닫기' };
        if (lang === TargetLanguage.English) return { title: 'Contact Us', subtitle: 'Feel free to reach out!', waDesc: 'Send us a message', line: 'Join Anonymous Line Community', lineDesc: 'Connect with fellow travelers', email: 'Email', close: 'Close' };
        if (lang === TargetLanguage.Thai) return { title: 'ติดต่อเรา', subtitle: 'มีคำถามอะไรติดต่อได้เลย!', waDesc: 'ส่งข้อความหาเรา', line: 'เข้าร่วมกลุ่ม Line แบบไม่ระบุตัวตน', lineDesc: 'แลกเปลี่ยนกับนักเดินทาง', email: 'อีเมล', close: 'ปิด' };
        if (lang === TargetLanguage.Vietnamese) return { title: 'Liên hệ', subtitle: 'Mọi thắc mắc xin liên hệ!', waDesc: 'Gửi tin nhắn cho chúng tôi', line: 'Tham gia cộng đồng Line ẩn danh', lineDesc: 'Kết nối với du khách', email: 'Email', close: 'Đóng' };
        return { title: '聯絡我們', subtitle: '有任何問題歡迎聯繫！', waDesc: '直接傳訊息給我們', line: '加入匿名 Line 社群', lineDesc: '與其他旅遊愛好者交流', email: '聯絡信箱', close: '關閉' };
    })();
    ...
    return (
        <div className="flex flex-col h-full relative overflow-hidden" style={{ background: isDarkMode ? '#1a1a1a' : '#fafaf9', color: isDarkMode ? '#fafaf9' : '#1c1917', transition: 'background 0.3s, color 0.3s' }}>

            {/* ========== 廣告推廣彈窗 ========== */}
            {showPromoPopup && typeof document !== 'undefined' && ReactDOM.createPortal(
                <>
                    <div
                    ...
                    <div
                        style={{
                            ...
                            zIndex: 99999, width: '90vw', maxWidth: '380px',
                            animation: 'promoPopupBounce 0.4s cubic-bezier(0.34, 1.56, 0.64, 1) forwards'
                        }}
                    >
                        <div style={{ background: 'white', borderRadius: '24px', boxShadow: '0 25px 60px rgba(0,0,0,0.3)', overflow: 'hidden' }}>
                            ...
                            <div style={{
                                ...
                                padding: '32px 24px 40px', textAlign: 'center', position: 'relative'
                            }}>
                                {/* 關閉按鈕 */}
                                <button
                                    onClick={() => setShowPromoPopup(false)}
                                ...
                                {/* APP LOGO (取代原本的 🎉 emoji) */}
                                <div style={{
                                    width: '72px', height: '72px', background: 'white', borderRadius: '18px',
                                    boxShadow: '0 8px 24px rgba(0,0,0,0.15)', margin: '0 auto 16px',
                                    display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden'
                                }}>
                                    <img src="/images/logo.png" alt="SausageMenuPal" style={{ width: '100%', height: '100%', objectFit: 'contain' }} />
                                </div>
                                <h2 style={{ color: 'white', fontSize: '22px', fontWeight: 800, margin: '0 0 4px' }}>{promoText.title}</h2>
                                <p style={{ color: 'rgba(255,255,255,0.8)', fontSize: '14px', margin: 0 }}>{promoText.subtitle}</p>
                            </div>

                            {/* iOS 開發中提示文字 */}
                            <div style={{
                                textAlign: 'center', padding: '14px 24px 0',
                                color: '#78716c', fontSize: '14px', fontWeight: 600
                            }}>
                                {promoText.iosDev}
                            </div>
                            ...
                            <div style={{ padding: '24px 24px 32px' }}>
                                {/* Android 下載 */}
                                <div style={{ background: '#f5f5f4', borderRadius: '16px', padding: '16px', marginBottom: '12px' }}>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '12px' }}>
                                        <div style={{
                                            width: '40px', height: '40px', background: '#dcfce7', borderRadius: '12px',
                                            display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '20px'
                                        }}>📱</div>
                                        <div>
                                            <p style={{ fontWeight: 700, color: '#292524', fontSize: '14px', margin: 0 }}>{promoText.androidTitle}</p>
                                            <p style={{ color: '#a8a29e', fontSize: '12px', margin: '2px 0 0' }}>{promoText.androidDesc}</p>
                                        </div>
                                    </div>
                                    <a
                                    ...
                                    >
                                        {promoText.downloadBtn}
                                    </a>
                                </div>

                                {/* Line 社群 */}
                                <div style={{ background: '#f5f5f4', borderRadius: '16px', padding: '16px', marginBottom: '16px' }}>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '12px' }}>
                                        <div style={{
                                            width: '40px', height: '40px', background: '#dcfce7', borderRadius: '12px',
                                            display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '20px'
                                        ...
                                        <div>
                                            <p style={{ fontWeight: 700, color: '#292524', fontSize: '14px', margin: 0 }}>{promoText.lineTitle}</p>
                                            <p style={{ color: '#a8a29e', fontSize: '12px', margin: '2px 0 0' }}>{promoText.lineDesc}</p>
                                        </div>
                                    </div>
                                    <a
                                        ...
                                        style={{
                                            display: 'block', width: '100%', textAlign: 'center',
                                            background: 'linear-gradient(to right, #06C755, #04B14F)',
                                            color: 'white', fontWeight: 700, padding: '12px 24px',
                                            borderRadius: '12px', textDecoration: 'none', fontSize: '15px',
            ...
            {/* 🌓 iOS-style Theme Toggle (Moved from header to floating below) */}
            <div className="absolute top-[88px] right-6 z-40 shadow-lg rounded-full pointer-events-auto">
                <button
                    ...
                    style={{
                        ...
                        border: 'none',
                        cursor: 'pointer',
                        background: isDarkMode ? '#1e293b' : '#f97316',
                ...
                >
                    ...
                    <Moon size={13} style={{
                        ...
                        color: isDarkMode ? '#94a3b8' : 'rgba(255,255,255,0.3)',
                        transition: 'color 0.3s'
                    }} />
                    {/* Sun icon (right) */}
                    <Sun size={13} style={{
                        position: 'absolute', right: '6px',
                        color: isDarkMode ? '#475569' : 'white',
                        transition: 'color 0.3s'
                    }} />
                    {/* Sliding knob */}
                    <div style={{
            ...
            {/* 🗺️ MAP 浮動按鈕 — 左側，與日夜切換上緣對齊 */}
            <div className="absolute top-[88px] left-6 z-40 shadow-lg rounded-full pointer-events-auto">
                <button
                    onClick={onOpenMap}
            ...
            <div className="flex-1 overflow-y-auto px-6 pb-20 space-y-6 z-10">
                ...
                {/* 這裡原本是 Pro 升級按鈕和免費次數，舊專案設定為全解鎖不需要顯示 */}
                    ...
                    >
                        <div style={{ background: 'white', borderRadius: '24px', boxShadow: '0 25px 60px rgba(0,0,0,0.3)', overflow: 'hidden' }}>
                            <div style={{
                                ...
                                <h2 style={{ color: 'white', fontSize: '20px', fontWeight: 800, margin: '0 0 4px' }}>{contactText.title}</h2>
                                <p style={{ color: 'rgba(255,255,255,0.8)', fontSize: '13px', margin: 0 }}>{contactText.subtitle}</p>
                            </div>
                            <div style={{ padding: '20px 20px 28px' }}>
                                <a href="https://wa.me/qr/KCBQ3XCKEFEWC1" target="_blank" rel="noopener noreferrer"
                                    style={{ display: 'flex', alignItems: 'center', gap: '14px', background: '#f5f5f4', borderRadius: '16px', padding: '14px 16px', marginBottom: '10px', textDecoration: 'none' }}>
                                    ...
                                    <div style={{ flex: 1 }}>
                                        ...
                                        <p style={{ color: '#a8a29e', fontSize: '12px', margin: '2px 0 0' }}>{contactText.waDesc}</p>
                                    </div>
                                </a>
                                <a href="https://reurl.cc/7bZXny" target="_blank" rel="noopener noreferrer"
                                    style={{ display: 'flex', alignItems: 'center', gap: '14px', background: '#f5f5f4', borderRadius: '16px', padding: '14px 16px', marginBottom: '10px', textDecoration: 'none' }}>
                                    <div style={{ width: '44px', height: '44px', background: '#06C755', borderRadius: '12px', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                                    ...
                                    <div style={{ flex: 1 }}>
                                        <p style={{ fontWeight: 700, color: '#292524', fontSize: '15px', margin: 0 }}>{contactText.line}</p>
                                        <p style={{ color: '#a8a29e', fontSize: '12px', margin: '2px 0 0' }}>{contactText.lineDesc}</p>
                                    </div>
                                </a>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '14px', background: '#f5f5f4', borderRadius: '16px', padding: '14px 16px', marginBottom: '16px' }}>
                                    <div style={{ width: '44px', height: '44px', background: '#f97316', borderRadius: '12px', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, fontSize: '20px' }}>📧</div>
                                    <div style={{ flex: 1 }}>
                                        <p style={{ fontWeight: 700, color: '#292524', fontSize: '13px', margin: 0 }}>{contactText.email}</p>
                                ...
                                <button onClick={() => setShowContactPopup(false)}
                                    style={{ display: 'block', width: '100%', textAlign: 'center', color: '#a8a29e', fontSize: '12px', background: 'none', border: 'none', cursor: 'pointer', padding: '4px' }}>
                                    {contactText.close}
                                </button>
                            </div>
                        </div>
