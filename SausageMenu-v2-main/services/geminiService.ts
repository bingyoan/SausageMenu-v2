...
const menuSchema: Schema = {
  ...
  properties: {
    ...
    restaurantCategory: { type: Type.STRING, description: "Infer the general category of the restaurant based on the food (e.g., Ramen, Cafe, Izakaya, Steakhouse, Thai, etc.). This MUST be translated into the target language." },
...
export const parseMenuImage = async (
  ...
  const prompt = `
    ...
    *** CONTEXTUAL TRANSLATION — MOST IMPORTANT ***
    ...
    6. CATEGORY: Infer the "restaurantCategory" based on the cuisine. Output in ${targetLanguage}.
  ...
  try {
    ...
    return {
      ...
      restaurantCategory: parsed.restaurantCategory,
  ...
  for (let i = 0; i < base64Images.length; i++) {
    ...
    const pagePrompt = `
      *** CRITICAL: ALL OUTPUT TEXT MUST BE IN ${targetLanguage} ***
      ...
      ${handwritingInstructions}
      ABSOLUTE REQUIREMENT: 
      - ALL item names MUST be translated to ${targetLanguage}
      - ALL descriptions MUST be in ${targetLanguage}
      - ALL category names MUST be in ${targetLanguage}
      - DO NOT use the original menu language in any text output
      ...
      *** CONTEXTUAL TRANSLATION — MOST IMPORTANT ***
      ...
      4. CURRENCY & EXCHANGE:
         ...
         - "exchangeRate" is an estimate of: 1 unit of Menu Currency = X units of ${targetCurrency}.
      5. DIETARY & ALLERGY: Detect allergens. Allergen names MUST be in ${targetLanguage}.
      6. CATEGORY: Infer the "restaurantCategory" based on the cuisine.
      Return pure JSON adhering to the schema.
    `;
    ...
    try {
      const response = await resilientFetch('/api/generate', {
        ...
        body: JSON.stringify({
          ...
          config: {
            responseMimeType: 'application/json',
            responseSchema: menuSchema,
            systemInstruction: `You are an expert menu digitizer. 100% recall. ALL text output MUST be in ${targetLanguage}.`
          }
        })
      });

      if (!response.ok) { console.error(`[Page ${i + 1}] Error`); continue; }
      ...
      if (isFirstPage) {
        ...
        finalCategory = parsed.restaurantCategory || '';
        ...
        try {
          ...
          if (rateRes.ok) {
            const rateData = await rateRes.json();
            if (rateData.rates) {
              const oRate = rateData.rates[finalCurrency];
              const tRate = rateData.rates[(targetCurrency || 'TWD').trim().toUpperCase()];
              if (oRate && tRate) finalRate = oRate / tRate;
            }
          }
        } catch (e) { console.warn("Rate fetch failed", e); }
      } else {
        if (!finalRestaurantName && parsed.restaurantName) finalRestaurantName = parsed.restaurantName;
        if (!finalCategory && parsed.restaurantCategory) finalCategory = parsed.restaurantCategory;
      }

      const pageItems = (parsed.items || []).map((item: any) => ({
        ...item, id: `item-${itemIdCounter++}-${Date.now()}`, category: item.category || 'General',
      }));
      allItems = [...allItems, ...pageItems];

      onPageComplete({
        items: allItems, originalCurrency: finalCurrency,
        targetCurrency: (targetCurrency || 'TWD').trim().toUpperCase(),
        exchangeRate: finalRate, detectedLanguage: finalDetectedLang, restaurantName: finalRestaurantName, restaurantCategory: finalCategory
      }, i, base64Images.length);
    } catch (error) { console.error(`[Page ${i + 1}]`, error); }
  }

  if (allItems.length === 0) throw new Error("No items could be extracted from any page.");
  return {
    items: allItems, originalCurrency: finalCurrency,
    targetCurrency: (targetCurrency || 'TWD').trim().toUpperCase(),
    exchangeRate: finalRate, detectedLanguage: finalDetectedLang, restaurantName: finalRestaurantName, restaurantCategory: finalCategory
