        ...
        {totalPages && totalPages > 1 ? (
          <>
            <div className="h-2 bg-sausage-200 rounded-full overflow-hidden">
              <motion.div className="h-full bg-sausage-600" initial={{ width: "0%" }} animate={{ width: `${(((currentPage ?? 0) + 1) / totalPages) * 100}%` }} transition={{ duration: 0.5 }} />
            </div>
            <div className="text-center">
              ...
              {(itemsFound ?? 0) > 0 && (
                <motion.p initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} className="text-sm text-sausage-600 mt-1">
          ...
          <>
            <div className="h-2 bg-sausage-200 rounded-full overflow-hidden">
              <motion.div className="h-full bg-sausage-600" initial={{ width: "0%" }} animate={{ width: `${((currentStep + 1) / STEPS.length) * 100}%` }} transition={{ duration: 0.5 }} />
            </div>
            <div className="h-8 relative overflow-hidden text-center">
              {STEPS.map((step, index) => (
                index === currentStep && (
                  <motion.p key={step} initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: -20, opacity: 0 }} className="absolute inset-0 w-full text-sausage-800 font-bold text-lg">
                    {step}
                  </motion.p>
                )
              ))}
