...
export const viewport: Viewport = {
  ...
  maximumScale: 1,
  userScalable: false,
  themeColor: '#ea580c', // 你原本的橘色主題
  colorScheme: 'light', // 🛑 警告瀏覽器：我們會自己處理深色模式，不要強制反轉顏色！
};
...
export default function RootLayout({
  ...
  return (
    <html lang="en" style={{ colorScheme: 'only light' }}>
      <head>
        {/* 🛡️ 最關鍵的 meta：阻止 Android Chrome 強制深色模式反轉顏色 */}
        <meta name="color-scheme" content="only light" />
        {/* Phosphor Icons 圖標庫 (保持不變) */}
        <script src="https://unpkg.com/@phosphor-icons/web" async></script>
      </head>
      <body className={`${nunito.className} antialiased h-screen selection:bg-sausage-200`} style={{ backgroundColor: '#fafaf9', color: '#1c1917' }}>
